// test/verify_gurgaon_two_products.js
...
async function runTests() {
  console.log('🚀 Running Gurgaon 2-Products Catalog Verification...\n');
  ...
  try {
    // 1. Authenticate Admin and Gurgaon Dealer
    console.log('1. Authenticating Admin and Gurgaon Dealer:');
    const adminLogin = await makeRequest('POST', '/auth/login', { username: 'admin', password: 'admin123' });
    assert.strictEqual(adminLogin.status, 200);
    const adminToken = adminLogin.data.token;
    console.log('  ✔ Admin logged in');

    const gurLogin = await makeRequest('POST', '/auth/login', { username: 'dealer_gurgaon', password: 'dealer123' });
    assert.strictEqual(gurLogin.status, 200);
    const gurToken = gurLogin.data.token;
    console.log('  ✔ Gurgaon Dealer logged in');

    const today = new Date().toISOString().slice(0, 10);

    // 2. Fetch District Products for Gurgaon
    console.log('\n2. Fetching Gurgaon Product Catalog:');
    const prodRes = await makeRequest('GET', '/inventory/district/Gurgaon', null, { 'Authorization': `Bearer ${gurToken}` });
    ...
    console.log('  ✔ Verified exactly 2 products in Gurgaon catalog:');
    products.forEach((p, idx) => {
      console.log(`    ${idx + 1}. ${p.name} (Stock: ${p.currentStock})`);
    });

    // 3. Verify District Day Stock Register
    console.log('\n3. Verifying Gurgaon Daily Stock Register:');
    const stockRes = await makeRequest('GET', `/inventory/district-day-stock/Gurgaon/${today}`, null, { 'Authorization': `Bearer ${gurToken}` });
    ...
    // 4. Gurgaon Dealer records a sale for AMAR NETRAN
    console.log('\n4. Recording a customer sale in Gurgaon:');
    const stockBefore = await makeRequest('GET', `/inventory/district-day-stock/Gurgaon/${today}`, null, { 'Authorization': `Bearer ${gurToken}` });
    ...
    const saleRes = await makeRequest(
      ...
      '/orders/create-order',
      {
        ...
        customerName: 'Gurgaon Test Customer',
        schemeName: `${p1.name} 1`
      },
      { 'Authorization': `Bearer ${gurToken}` }
    );
    ...
    const stockAfter = await makeRequest('GET', `/inventory/district-day-stock/Gurgaon/${today}`, null, { 'Authorization': `Bearer ${gurToken}` });
    ...
    await initDb();
    const persistedGurProds = db.districtProducts['Gurgaon'];
    assert.strictEqual(persistedGurProds.length, 2, 'Gurgaon products must remain exactly 2 across server restarts');
    console.log('  ✔ Verified: Gurgaon 2-products catalog persisted in PostgreSQL!');

    console.log('\n🎉 GURGAON 2-PRODUCTS CATALOG VERIFIED 100%!');
    process.exit(0);
  } catch (err) {
